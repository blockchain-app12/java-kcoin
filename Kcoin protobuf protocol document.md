...
This is the description of  Google Protobuf implementation of Kcoin's protocol.
...
- Kcoin has 3 `types` of account: Normal, AssetIssue, Contract
...
- message `Account` has multiple attributes and 2 nested messages: 
  ...
  `balance`: the SYM balance of this account.

  `votes`: received votes of this account. – e.g. *{(“0x1b7w…9xj3”,323), (“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}*.

  `asset`: other assets except SYM in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*.

  `assetV2`: other assets except SYM in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*. (used after allowing same name of token87)

  `frozen`: the freezed SYM of this account for receiving bandwidth
...
- message `TransactionRet`
  ...
  - message `ResourceReceipt`
    ...
    `net_fee`: consume yourself sym of net usage.
  ...
  - Message `InternalTransaction`
    ...
    `callValueInfo`: Refers to asset transfer information in internal transactions, including sym and trc10.
  ...
  - message `TransactionInfo`
   ...
   `unfreeze_amount`: unfreeze sym amount.
...
- Kcoin has 33 types of Contracts declared within [`Transaction`](#trans).
...
- There are 15 types of results while deploying contracts (refer to `Transaction.Result`):
  ...
  - message `VoteWitnessContract`
    ...
    - message `AssetIssueContract`
      ...
      `sym_num`: sym num defines token price.

      `precision`: precision.

      `num`: sym num defines token price.
    ...
    - message `FreezeBalanceContract`
      ...
      `frozen_balance`: frozen amount of SYM.

      `frozen_duration`: frozen duration of SYM.

      `resource`: type of resource gained from freezing SYM.
    ...
    - message `UnfreezeBalanceContract`
     ...
     - message `TriggerSmartContract`
       ...
       `call_value`: SYM amount sent to smart contract.
...
- message `SmartContract`
  ...
  `call_value`: amount of SYM that send to the smart contract.
  ...
  `sym_hash`:  transactionId of Deploying contract transaction.
