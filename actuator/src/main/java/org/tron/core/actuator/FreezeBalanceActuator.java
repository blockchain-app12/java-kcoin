...
public class FreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    switch (freezeBalanceContract.getResource()) {
      case BANDWIDTH:
        ...
        dynamicStore
            .addTotalNetWeight(frozenBalance / SYM_PRECISION);
        break;
      case ENERGY:
        if (!ArrayUtils.isEmpty(receiverAddress)
            && dynamicStore.supportDR()) {
        ...
        dynamicStore
            .addTotalEnergyWeight(frozenBalance / SYM_PRECISION);
        break;
      case KCOIN_POWER:
        long newFrozenBalanceForKcoinPower =
            frozenBalance + accountCapsule.getKcoinPowerFrozenBalance();
        accountCapsule.setFrozenForKcoinPower(newFrozenBalanceForKcoinPower, expireTime);

        dynamicStore
            .addTotalKcoinPowerWeight(frozenBalance / SYM_PRECISION);
        break;
      default:
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (frozenBalance < SYM_PRECISION) {
      throw new ContractValidateException("frozenBalance must be more than 1SYM");
    }
    ...
    switch (freezeBalanceContract.getResource()) {
      ...
      case KCOIN_POWER:
        if (dynamicStore.supportAllowNewResourceModel()) {
          ...
          if (!ArrayUtils.isEmpty(receiverAddress)) {
            throw new ContractValidateException(
                "KCOIN_POWER is not allowed to delegate to other accounts.");
          }
        } else {
          throw new ContractValidateException(
