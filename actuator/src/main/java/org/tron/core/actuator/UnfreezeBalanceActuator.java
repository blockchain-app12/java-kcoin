...
import static org.kcoin.core.actuator.ActuatorConstant.ACCOUNT_EXCEPTION_STR;
...
public class UnfreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && accountCapsule.oldKcoinPowerIsNotInitialized()) {
      accountCapsule.initializeOldKcoinPower();
    }
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case ENERGY:
          ...
          break;
        case KCOIN_POWER:
          unfreezeBalance = accountCapsule.getKcoinPowerFrozenBalance();
          accountCapsule.setInstance(accountCapsule.getInstance().toBuilder()
              .setBalance(oldBalance + unfreezeBalance)
              .clearKcoinPower().build());
          break;
        default:
          //this should never happen
          break;
    ...
    switch (unfreezeBalanceContract.getResource()) {
      case BANDWIDTH:
        dynamicStore
            .addTotalNetWeight(-unfreezeBalance / SYM_PRECISION);
        break;
      case ENERGY:
        dynamicStore
            .addTotalEnergyWeight(-unfreezeBalance / SYM_PRECISION);
        break;
      case KCOIN_POWER:
        dynamicStore
            .addTotalKcoinPowerWeight(-unfreezeBalance / SYM_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && accountCapsule.oldKcoinPowerIsInvalid()) {
      switch (unfreezeBalanceContract.getResource()) {
        case BANDWIDTH:
        case ENERGY:
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && !accountCapsule.oldKcoinPowerIsInvalid()) {
      accountCapsule.invalidateOldKcoinPower();
    }
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case KCOIN_POWER:
          if (dynamicStore.supportAllowNewResourceModel()) {
            Frozen frozenBalanceForKcoinPower = accountCapsule.getInstance().getKcoinPower();
            if (frozenBalanceForKcoinPower.getFrozenBalance() <= 0) {
              throw new ContractValidateException("no frozenBalance(KcoinPower)");
            }
            if (frozenBalanceForKcoinPower.getExpireTime() > now) {
              throw new ContractValidateException("It's not time to unfreeze(KcoinPower).");
            }
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
        ...
        default:
          if (dynamicStore.supportAllowNewResourceModel()) {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy、KCOIN_POWER]");
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
          }
