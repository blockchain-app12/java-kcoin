...
import static org.kcoin.core.vm.utils.MUtil.transfer;
import static org.kcoin.core.vm.utils.MUtil.transferToken;
...
import org.kcoin.common.runtime.InternalTransaction.ExecutorType;
...
import org.kcoin.core.vm.program.Program.TransferException;
import org.kcoin.core.vm.program.ProgramPrecompile;
...
public class VMActuator implements Actuator2 {
  ...
  @Getter
  @Setter
  private InternalTransaction.SymType symType;
  ...
  public void validate(Object object) throws ContractValidateException {
    ...
    sym = context.getSymCap().getInstance();
    blockCap = context.getBlockCap();
    if (VMConfig.allowTvmFreeze() && context.getSymCap().getSymTrace() != null) {
      receipt = context.getSymCap().getSymTrace().getReceipt();
    }
    //Route Type
    ContractType contractType = this.sym.getRawData().getContract(0).getType();
    //Prepare Repository
    ...
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        symType = SymType.SYM_CONTRACT_CALL_TYPE;
        call();
        break;
      case ContractType.CreateSmartContract_VALUE:
        symType = SymType.SYM_CONTRACT_CREATION_TYPE;
        create();
        break;
      default:
    ...
    try {
      if (vm != null) {
        if (null != blockCap && blockCap.generatedByMyself && blockCap.hasWitnessSignature()
            && null != TransactionUtil.getContractRet(sym)
            && contractResult.OUT_OF_TIME == TransactionUtil.getContractRet(sym)) {
        ...
        if (isConstantCall) {
          long callValue = TransactionCapsule.getCallValue(sym.getRawData().getContract(0));
          long callTokenValue = TransactionUtil
              .getCallTokenValue(sym.getRawData().getContract(0));
        ...
        if (SymType.SYM_CONTRACT_CREATION_TYPE == symType && !result.isRevert()) {
  ...
  private void create()
    ...
    CreateSmartContract contract = ContractCapsule.getSmartContractFromTransaction(sym);
    if (contract == null) {
    ...
    byte[] contractAddress = WalletUtil.generateContractAddress(sym);
    ...
    try {
      long feeLimit = sym.getRawData().getFeeLimit();
      ...
      rootInternalTransaction = new InternalTransaction(sym, symType);
      ...
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(SymType.SYM_CONTRACT_CREATION_TYPE, executorType, sym,
      ...
      byte[] txId = TransactionUtil.getTransactionId(sym).getBytes();
  ...
  private void call()
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(SymType.SYM_CONTRACT_CALL_TYPE, executorType, sym,
              tokenValue, tokenId, blockCap.getInstance(), repository, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isConstantCall) {
  ...
  private double getCpuLimitInUsRatio() {
    ...
    if (ExecutorType.ET_NORMAL_TYPE == executorType) {
      ...
      if (this.blockCap != null && blockCap.generatedByMyself &&
        ...
        if (sym.getRet(0).getContractRet() == contractResult.OUT_OF_TIME) {
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit SYM when the creator's frozen energy is 0.
