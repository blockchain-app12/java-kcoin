  ...
  public boolean validate() throws ContractValidateException {
    ...
    try {
      ...
      if (dynamicStore.supportAllowNewResourceModel()) {
        kcoinPower = accountCapsule.getAllKcoinPower();
      } else {
        kcoinPower = accountCapsule.getKcoinPower();
      }

      sum = LongMath
          .checkedMultiply(sum, SYM_PRECISION); //sym -> drop. The vote count is based on SYM
      if (sum > kcoinPower) {
        throw new ContractValidateException(
