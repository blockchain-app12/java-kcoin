...
import org.kcoin.api.GrpcAPI.TransactionSignWeight.Result;
...
import org.kcoin.protos.Protocol.Permission.PermissionType;
...
public class TransactionUtil {
  ...
  public static TransactionCapsule getTransactionSign(TransactionSign transactionSign) {
    ...
    TransactionCapsule sym = new TransactionCapsule(transactionSign.getTransaction());
    sym.sign(privateKey);
    return sym;
  }
  ...
  public TransactionCapsule addSign(TransactionSign transactionSign)
    ...
    byte[] privateKey = transactionSign.getPrivateKey().toByteArray();
    TransactionCapsule sym = new TransactionCapsule(transactionSign.getTransaction());
    sym.addSign(privateKey, chainBaseManager.getAccountStore());
    return sym;
  }

  public TransactionSignWeight getTransactionSignWeight(Transaction sym) {
    ...
    try {
      ...
      if (sym.getSignatureCount() > 0) {
        ...
        long currentWeight = TransactionCapsule.checkWeight(permission, sym.getSignatureList(),
            Sha256Hash.hash(CommonParameter.getInstance()
                .isECKeyCryptoEngine(), sym.getRawData().toByteArray()), approveList);
