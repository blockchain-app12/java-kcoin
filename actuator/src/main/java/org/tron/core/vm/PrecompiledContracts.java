...
import static org.kcoin.common.runtime.vm.DataWord.WORD_SIZE;
import static org.kcoin.common.utils.BIUtil.addSafely;
import static org.kcoin.common.utils.BIUtil.isLessThan;
import static org.kcoin.common.utils.BIUtil.isZero;
import static org.kcoin.common.utils.ByteUtil.*;
import static org.kcoin.core.db.TransactionTrace.convertToKcoinAddress;
...
import org.kcoin.common.crypto.SignatureInterface;
import org.kcoin.common.crypto.zksnark.BN128;
import org.kcoin.common.crypto.zksnark.BN128Fp;
import org.kcoin.common.crypto.zksnark.BN128G1;
import org.kcoin.common.crypto.zksnark.BN128G2;
import org.kcoin.common.crypto.zksnark.Fp;
import org.kcoin.common.crypto.zksnark.PairingCheck;
...
import org.kcoin.common.zksnark.LibrustzcashParam;
...
public class PrecompiledContracts {
  ...
  public static class ValidateMultiSign extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] rawData) {
      ...
      byte[] combine = ByteUtil
          .merge(convertToKcoinAddress(addr), ByteArray.fromInt(permissionId), data);
      ...
      AccountCapsule account = this.getDeposit().getAccount(convertToKcoinAddress(addr));
      if (account != null) {
        try {
  ...
  public static class RewardBalance extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      long rewardBalance =
          VoteRewardUtil.queryReward(convertToKcoinAddress(callerAddress), getDeposit());
  ...
  public static class IsSrCandidate extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      WitnessCapsule witnessCapsule = this.getDeposit()
          .getWitness(convertToKcoinAddress(addr));
  ...
  public static class VoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      DataWord[] words = DataWord.parseArray(data);
      byte[] voteKcoinAddr = convertToKcoinAddress(words[0].getLast20Bytes());
      byte[] targetKcoinAddr = convertToKcoinAddress(words[1].getLast20Bytes());

      long voteCount = 0;
      AccountCapsule voteAccountCapsule = this.getDeposit().getAccount(voteKcoinAddr);
      if (voteAccountCapsule != null && !voteAccountCapsule.getVotesList().isEmpty()) {
        ...
        for (Protocol.Vote vote : voteList) {
          if (ByteString.copyFrom(targetKcoinAddr).equals(vote.getVoteAddress())) {
  ...
  public static class ReceivedVoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      DataWord[] words = DataWord.parseArray(data);
      byte[] targetKcoinAddr = convertToKcoinAddress(words[0].getLast20Bytes());

      long voteCount = 0;
      WitnessCapsule witnessCapsule =
          this.getDeposit().getWitness(targetKcoinAddr);
      if (witnessCapsule != null) {
  ...
  public static class TotalVoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      AccountCapsule accountCapsule = this.getDeposit().getAccount(voteKcoinAddr);
      if (accountCapsule != null) {
        return Pair.of(true,
            longTo32Bytes(accountCapsule.getKcoinPower() / SYM_PRECISION));
      }
