...
public class UnfreezeBalanceProcessor {
  ...
  public long execute(UnfreezeBalanceParam param, Repository repo) {
    ...
    switch (param.getResourceType()) {
      case BANDWIDTH:
        repo.addTotalNetWeight(-unfreezeBalance / SYM_PRECISION);
        break;
      case ENERGY:
        repo.addTotalEnergyWeight(-unfreezeBalance / SYM_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (VMConfig.allowTvmVote() && !accountCapsule.getVotesList().isEmpty()) {
      long usedKcoinPower = 0;
      for (Protocol.Vote vote : accountCapsule.getVotesList()) {
        usedKcoinPower += vote.getVoteCount();
      }
      if (accountCapsule.getKcoinPower() < usedKcoinPower * SYM_PRECISION) {
