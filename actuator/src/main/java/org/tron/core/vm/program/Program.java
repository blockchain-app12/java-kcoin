...
package org.kcoin.core.vm.program;
...
import static org.kcoin.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.kcoin.common.utils.BIUtil;
import org.kcoin.common.utils.ByteUtil;
import org.kcoin.common.utils.FastByteComparisons;
...
import org.kcoin.core.exception.KcoinException;
import org.kcoin.core.utils.TransactionUtil;
import org.kcoin.core.vm.EnergyCost;
import org.kcoin.core.vm.MessageCall;
import org.kcoin.core.vm.OpCode;
import org.kcoin.core.vm.PrecompiledContracts;
import org.kcoin.core.vm.VM;
import org.kcoin.core.vm.VMConstant;
import org.kcoin.core.vm.VMUtils;
import org.kcoin.core.vm.config.VMConfig;
import org.kcoin.core.vm.nativecontract.FreezeBalanceProcessor;
import org.kcoin.core.vm.nativecontract.UnfreezeBalanceProcessor;
import org.kcoin.core.vm.nativecontract.VoteWitnessProcessor;
import org.kcoin.core.vm.nativecontract.WithdrawRewardProcessor;
import org.kcoin.core.vm.nativecontract.param.FreezeBalanceParam;
import org.kcoin.core.vm.nativecontract.param.UnfreezeBalanceParam;
import org.kcoin.core.vm.nativecontract.param.VoteWitnessParam;
import org.kcoin.core.vm.nativecontract.param.WithdrawRewardParam;
import org.kcoin.core.vm.program.invoke.ProgramInvoke;
import org.kcoin.core.vm.program.invoke.ProgramInvokeFactory;
import org.kcoin.core.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.kcoin.core.vm.program.listener.CompositeProgramListener;
import org.kcoin.core.vm.program.listener.ProgramListenerAware;
import org.kcoin.core.vm.program.listener.ProgramStorageChangeListener;
import org.kcoin.core.vm.repository.Repository;
import org.kcoin.core.vm.trace.ProgramTrace;
import org.kcoin.core.vm.trace.ProgramTraceListener;
import org.kcoin.core.vm.utils.MUtil;
...
import org.kcoin.protos.contract.Common;
import org.kcoin.protos.contract.SmartContractOuterClass.SmartContract;
import org.kcoin.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send SYM to.
   * @param value the SYM value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToKcoinAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToKcoinAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all sym failed in suicide: %s", e.getMessage());
        }
  ...
  private void transferDelegatedResourceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    ...
    repo.addTotalNetWeight(-frozenBalanceForBandwidthOfOwner / SYM_PRECISION);

    long frozenBalanceForEnergyOfOwner =
        ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance();
    repo.addTotalEnergyWeight(-frozenBalanceForEnergyOfOwner / SYM_PRECISION);
...
//    return freezeCheck && voteCheck;
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    byte[] senderAddress = TransactionTrace
        .convertToKcoinAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setSymHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(
                TransactionTrace.convertToKcoinAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace
        .convertToKcoinAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace
        .convertToKcoinAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer sym failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(
            TransactionTrace.convertToKcoinAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit()
        .getCode(TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] kcoinAddr = TransactionTrace.convertToKcoinAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(kcoinAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(kcoinAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(kcoinAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord getRewardBalance(DataWord address) {
    long rewardBalance = VoteRewardUtil.queryReward(
        TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()));
  ...
  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState()
            .getWitness(TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()));
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToKcoinAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if(VMConfig.allowTvmIstanbul()) {
      ...
      senderAddress = TransactionTrace
          .convertToKcoinAddress(this.getCallerAddress().getLast20Bytes());
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToKcoinAddress(msg.getType().callIsDelegate()
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or SYM to non-exist address(would create one)
  ...
  public boolean freeze(DataWord receiverAddress, DataWord frozenBalance, DataWord resourceType) {
    ...
    byte[] owner = TransactionTrace.convertToKcoinAddress(getContractAddress().getLast20Bytes());
    byte[] receiver = TransactionTrace.convertToKcoinAddress(receiverAddress.getLast20Bytes());
  ...
  public long freezeExpireTime(DataWord targetAddress, DataWord resourceType) {
    byte[] owner = TransactionTrace.convertToKcoinAddress(getContractAddress().getLast20Bytes());
    byte[] target = TransactionTrace.convertToKcoinAddress(targetAddress.getLast20Bytes());
    ...
    try {
      ...
      for (int i = 0; i < witnessArrayLength; i++) {
        ...
        DataWord amount = new DataWord(Arrays.copyOfRange(amountArrayData,
            i * DataWord.WORD_SIZE, (i + 1) * DataWord.WORD_SIZE));
        param.addVote(TransactionTrace.convertToKcoinAddress(witness.getLast20Bytes()),
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(KcoinException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(KcoinException e) {
      return new PrecompiledContractException(e.getMessage());
    }
