...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] symHash) {
    if (ByteUtil.isNullOrZeroArray(symHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, symHash));
  }

  public void generateAddrHash(byte[] symId) {
    // update addreHash for create2
    addrHash = addrHash(address, symId);
  }
