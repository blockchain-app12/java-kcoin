...
public class InternalTransaction {
  ...
  /* the amount of sym to transfer (calculated as sun) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction sym, InternalTransaction.SymType symType)
      throws ContractValidateException {
    this.transaction = sym;
    TransactionCapsule symCap = new TransactionCapsule(sym);
    this.protoEncoded = symCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (symType == SymType.SYM_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(sym);
      this.note = "create";
    ...
    } else if (symType == SymType.SYM_CONTRACT_CALL_TYPE) {
    ...
    this.hash = symCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a sym callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. sym or a token. can't be both. We should avoid using
  ...
  public enum SymType {
    SYM_PRECOMPILED_TYPE,
    SYM_CONTRACT_CREATION_TYPE,
    SYM_CONTRACT_CALL_TYPE,
    SYM_UNKNOWN_TYPE,
  }
