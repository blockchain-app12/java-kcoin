...
import org.kcoin.protos.Protocol.Account.AccountResource;
import org.kcoin.protos.Protocol.Account.Builder;
import org.kcoin.protos.Protocol.Account.Frozen;
...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  public long getKcoinPowerUsage() {
    if (this.account.getVotesList() != null) {
  ...
  public long getAllKcoinPower() {
    if (account.getOldKcoinPower() == -1) {
      return getKcoinPowerFrozenBalance();
    } else if (account.getOldKcoinPower() == 0) {
      return getKcoinPower() + getKcoinPowerFrozenBalance();
    } else {
      return account.getOldKcoinPower() + getKcoinPowerFrozenBalance();
    }
  }
  ...
  public boolean oldKcoinPowerIsNotInitialized() {
    return this.account.getOldKcoinPower() == 0;
  }

  public boolean oldKcoinPowerIsInvalid() {
    return this.account.getOldKcoinPower() == -1;
  }

  public void initializeOldKcoinPower() {
    long value = getKcoinPower();
    if (value == 0) {
      value = -1;
    }
    setInstance(getInstance().toBuilder()
        .setOldKcoinPower(value)
        .build());
  }

  public void invalidateOldKcoinPower() {
    setInstance(getInstance().toBuilder()
        .setOldKcoinPower(-1)
        .build());
  }
  ...
  public void setOldKcoinPower(long value) {
    setInstance(getInstance().toBuilder()
        .setOldKcoinPower(value)
        .build());
  }

  public void setFrozenForKcoinPower(long frozenBalance, long expireTime) {
    Frozen newFrozen = Frozen.newBuilder()
        .setFrozenBalance(frozenBalance)
        .setExpireTime(expireTime)
        .build();
    ...
    setInstance(getInstance().toBuilder()
        .setKcoinPower(newFrozen)
        .build());
  }

  public long getKcoinPowerFrozenBalance() {
    return this.account.getKcoinPower().getFrozenBalance();
  }
