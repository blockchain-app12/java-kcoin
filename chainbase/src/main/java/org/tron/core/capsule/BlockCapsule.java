...
import org.kcoin.core.capsule.utils.MerkleTree;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public BlockCapsule(long timestamp, ByteString parentHash, long number,
    ...
    // block
    Block.Builder blockBuild = Block.newBuilder();
    transactionList.forEach(sym -> blockBuild.addTransactions(sym));
    this.block = blockBuild.setBlockHeader(blockHeader).build();
    initTxs();
  }
  ...
  public void addTransaction(TransactionCapsule pendingSym) {
    this.block = this.block.toBuilder().addTransactions(pendingSym.getInstance()).build();
    getTransactions().add(pendingSym);
  }
  ...
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(sym -> new TransactionCapsule(sym))
