...
import org.kcoin.protos.contract.SmartContractOuterClass.SmartContractDataWrapperOrBuilder;
...
public class ContractCapsule implements ProtoCapsule<SmartContract> {
  ...
  public static CreateSmartContract getSmartContractFromTransaction(Transaction sym) {
    try {
      Any any = sym.getRawData().getContract(0).getParameter();
  ...
  public static TriggerSmartContract getTriggerContractFromTransaction(Transaction sym) {
    try {
      Any any = sym.getRawData().getContract(0).getParameter();
  ...
  public byte[] getSymHash() {
    return this.smartContract.getSymHash().toByteArray();
  }
}
