...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isSYM, long quant) {
    logger.info("isSYM: " + isSYM);
    long balance = isSYM ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isSYM, long quant) {
    logger.info("isSYM: " + isSYM);
    long balance = isSYM ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isSYM, long supplyQuant) {
    long balance = isSYM ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isSYM) {
    long relay = exchangeToSupply(isSYM, from);
    return exchange_from_supply(!isSYM, relay);
  }
