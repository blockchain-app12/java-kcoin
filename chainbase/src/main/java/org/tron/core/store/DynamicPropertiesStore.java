...
import org.kcoin.core.config.Parameter;
import org.kcoin.core.config.Parameter.ChainConstant;
import org.kcoin.core.db.KcoinStoreWithRevoking;

@Slf4j(topic = "DB")
@Component
public class DynamicPropertiesStore extends KcoinStoreWithRevoking<BytesCapsule> {
  ...
  private static final byte[] BURN_SYM_AMOUNT = "BURN_SYM_AMOUNT".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      this.getTotalKcoinPowerWeight();
    } catch (IllegalArgumentException e) {
      this.saveTotalKcoinPowerWeight(0L);
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(100_000L); // 0.1SYM
    }
  ...
  public void saveTotalKcoinPowerWeight(long totalEnergyWeight) {
    this.put(DynamicResourceProperties.TOTAL_KCOIN_POWER_WEIGHT,
        new BytesCapsule(ByteArray.fromLong(totalEnergyWeight)));
  }

  public long getTotalKcoinPowerWeight() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KCOIN_POWER_WEIGHT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KCOIN_POWER_WEIGHT"));
  }
  ...
  public void saveTotalStoragePool(long sym) {
    this.put(TOTAL_STORAGE_POOL,
        new BytesCapsule(ByteArray.fromLong(sym)));
  }
  ...
  public void saveTotalStorageTax(long sym) {
  ...
  //The unit is sym
  public void addTotalKcoinPowerWeight(long amount) {
    long totalWeight = getTotalKcoinPowerWeight();
    totalWeight += amount;
    saveTotalKcoinPowerWeight(totalWeight);
  }
  ...
  public long getBurnSymAmount() {
    return Optional.ofNullable(getUnchecked(BURN_SYM_AMOUNT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(() -> new IllegalArgumentException("not found BURN_SYM_AMOUNT"));
  }

  public void burnSym(long amount) {
    if (amount <= 0) {
      return;
    }
    amount += getBurnSymAmount();
    saveBurnSym(amount);
  }

  private void saveBurnSym(long amount) {
    this.put(BURN_SYM_AMOUNT, new BytesCapsule(ByteArray.fromLong(amount)));
  }
  ...
  private static class DynamicResourceProperties {
    ...
    private static final byte[] TOTAL_KCOIN_POWER_WEIGHT = "TOTAL_KCOIN_POWER_WEIGHT".getBytes();
