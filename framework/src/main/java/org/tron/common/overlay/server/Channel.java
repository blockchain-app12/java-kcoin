package org.kcoin.common.overlay.server;
...
import org.kcoin.common.overlay.message.HelloMessage;
import org.kcoin.common.overlay.message.MessageCodec;
import org.kcoin.common.overlay.message.StaticMessages;
import org.kcoin.core.db.ByteArrayWrapper;
import org.kcoin.core.exception.P2pException;
import org.kcoin.core.net.PbftHandler;
import org.kcoin.core.net.KcoinNetHandler;
...
public class Channel {
  ...
  @Autowired
  private KcoinNetHandler kcoinNetHandler;
  @Autowired
  ...
  private KcoinState kcoinState = KcoinState.INIT;
  ...
  public void init(ChannelPipeline pipeline, String remoteId, boolean discoveryMode,
    ...
    pipeline.addLast("lengthDecode", new SymProtobufVarint32FrameDecoder(this));
    ...
    kcoinNetHandler.setChannel(this);
    pbftHandler.setChannel(this);

    p2pHandler.setMsgQueue(msgQueue);
    kcoinNetHandler.setMsgQueue(msgQueue);
  ...
  public void publicHandshakeFinished(ChannelHandlerContext ctx, HelloMessage msg) {
    ...
    ctx.pipeline().addLast("data", kcoinNetHandler);
    ctx.pipeline().addLast("pbft", pbftHandler);
    setStartTime(msg.getTimestamp());
    setKcoinState(KcoinState.HANDSHAKE_FINISHED);
  ...
  public void setKcoinState(KcoinState kcoinState) {
    this.kcoinState = kcoinState;
    logger.info("Peer {} status change to {}.", inetSocketAddress, kcoinState);
  }
