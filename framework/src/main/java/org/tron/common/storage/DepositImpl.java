...
import org.kcoin.core.vm.repository.Type;
...
public class DepositImpl implements Deposit {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getSymHash())) {
      storage.generateAddrHash(contract.getSymHash());
    }
    return storage;
  }
  ...
  public synchronized void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = convertToKcoinAddress(address);
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] symHash) {
    Key key = Key.create(symHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(symHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(symHash);
