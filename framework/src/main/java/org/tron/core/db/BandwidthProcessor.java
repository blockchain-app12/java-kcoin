...
import static org.kcoin.protos.Protocol.Transaction.Contract.ContractType.ShieldedTransferContract;
...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule sym, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException,
      TooBigTransactionResultException {
    List<Contract> contracts = sym.getInstance().getRawData().getContractList();
    if (sym.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = sym.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = sym.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("symId {}, bandwidth cost: {}", sym.getTransactionId(), bytesSize);
  ...
  public long calculateGlobalNetLimit(AccountCapsule accountCapsule) {
    ...
    if (frozeBalance < SYM_PRECISION) {
      return 0;
    }
    long netWeight = frozeBalance / SYM_PRECISION;
