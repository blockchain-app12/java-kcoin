...
public class GetTransactionInfoByIdServlet extends RateLimiterServlet {
  ...
  private static String convertLogAddressToKcoinAddress(TransactionInfo transactionInfo,
      boolean visible) {
    if (visible) {
      List<Log> newLogList = Util.convertLogAddressToKcoinAddress(transactionInfo);
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (reply != null) {
        response.getWriter().println(convertLogAddressToKcoinAddress(reply, visible));
      } else {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (reply != null) {
        response.getWriter().println(convertLogAddressToKcoinAddress(reply, params.isVisible()));
      } else {
        response.getWriter().println("{}");
      }
    } catch (Exception e) {
