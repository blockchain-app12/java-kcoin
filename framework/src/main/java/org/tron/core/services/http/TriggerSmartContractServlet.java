...
public class TriggerSmartContractServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      TransactionCapsule symCap = wallet
          .createTransactionCapsule(build.build(), ContractType.TriggerSmartContract);

      Transaction.Builder txBuilder = symCap.getInstance().toBuilder();
      Transaction.raw.Builder rawBuilder = symCap.getInstance().getRawData().toBuilder();
      rawBuilder.setFeeLimit(feeLimit);
      txBuilder.setRawData(rawBuilder);

      Transaction sym = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), symExtBuilder,
              retBuilder);
      sym = Util.setTransactionPermissionId(jsonObject, sym);
    ...
    response.getWriter().println(Util.printTransactionExtention(symExtBuilder.build(), visible));
  }
}
