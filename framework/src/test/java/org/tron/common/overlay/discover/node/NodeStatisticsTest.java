package org.kcoin.common.overlay.discover.node;
...
import org.kcoin.common.overlay.discover.node.statistics.MessageStatistics;
import org.kcoin.common.overlay.discover.node.statistics.NodeStatistics;
import org.kcoin.common.overlay.message.DisconnectMessage;
import org.kcoin.common.overlay.message.PongMessage;
...
public class NodeStatisticsTest {
  ...
  public void testMessage() {
    ...
    Assert.assertEquals(1, statistics.kcoinInSyncBlockChain.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.kcoinOutBlockChainInventory.getTotalCount());

    InventoryMessage invMsgSym =
        new InventoryMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.SYM);
    String inventoryMessageStr = invMsgSym.toString();
    Assert.assertNotNull(inventoryMessageStr);
    statistics.addTcpInMessage(invMsgSym);
    statistics.addTcpOutMessage(invMsgSym);
    ...
    Assert.assertEquals(1, statistics.kcoinInBlockInventory.getTotalCount());

    FetchInvDataMessage fetchInvDataSym =
        new FetchInvDataMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.SYM);
    statistics.addTcpInMessage(fetchInvDataSym);
    statistics.addTcpOutMessage(fetchInvDataSym);
    ...
    Assert.assertEquals(1, statistics.kcoinInSymFetchInvData.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.kcoinInSyms.getTotalCount());
    ...
    long inBlockCount = statistics.kcoinInBlock.getTotalCount();
