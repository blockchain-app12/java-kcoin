...
import static org.kcoin.core.capsule.utils.TransactionUtil.buildTransactionInfoInstance;
import static org.kcoin.core.utils.TransactionUtil.generateContractAddress;
...
public class ProgramResultTest {
  ...
  public void successAndFailResultTest()
    ...
    TransactionTrace traceSuccess = TvmTestUtils
        .processTransactionAndReturnTrace(sym1, deposit, null);
    ...
    checkTransactionInfo(traceSuccess, sym1, null, internalTransactionsList);
    ...
    Transaction sym2 = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = TvmTestUtils
        .processTransactionAndReturnTrace(sym2, deposit, null);
    runtime = traceFailed.getRuntime();

    byte[] bContract2 =
        generateContractAddress(new TransactionCapsule(sym2).getTransactionId().getBytes(), 0);
    ...
    checkTransactionInfo(traceFailed, sym2, null, internalTransactionsListFail);
  }
  ...
  public void timeOutFeeTest()
    ...
    TransactionInfoCapsule symInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(sym1), null, traceSuccess);
    Assert.assertEquals(symInfoCapsule.getFee(), 12705900L);
    Assert.assertEquals(symInfoCapsule.getPackingFee(), 0L);
    ...
    symInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(sym1), null, traceSuccess);
    Assert.assertEquals(symInfoCapsule.getFee(), 12705900L);
    Assert.assertEquals(symInfoCapsule.getPackingFee(), 12705900L);
  ...
  public void suicideResultTest()
    ...
    Transaction sym = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = TvmTestUtils.processTransactionAndReturnTrace(sym, deposit, null);
    ...
    Assert.assertEquals(internalTransactionsList.get(0).isRejected(), false);
    checkTransactionInfo(trace, sym, null, internalTransactionsList);
  }
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction sym, BlockCapsule block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoCapsule symInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(sym), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = symInfoCapsule
