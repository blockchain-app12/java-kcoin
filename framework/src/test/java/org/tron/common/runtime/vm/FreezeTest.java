...
import static org.kcoin.core.config.Parameter.ChainConstant.FROZEN_PERIOD;
import static org.kcoin.core.config.Parameter.ChainConstant.SYM_PRECISION;
import static org.kcoin.protos.Protocol.Transaction.Result.contractResult.REVERT;
...
public class FreezeTest {
  ...
  private byte[] deployContract(byte[] deployer,
    ...
    Protocol.Transaction sym = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, deployer, "[]", code, value, fee, consumeUserResourcePercent,
        null, originEnergyLimit);
    byte[] contractAddr = WalletUtil.generateContractAddress(sym);
    //String contractAddrStr = StringUtil.encode58Check(contractAddr);
    TransactionCapsule symCap = new TransactionCapsule(sym);
  ...
  private TVMTestResult triggerContract(byte[] callerAddr,
    ...
    TransactionCapsule symCap = new TransactionCapsule(
  ...
  private byte[] getCreate2Addr(byte[] factoryAddr,
    ...
    return TransactionTrace.convertToKcoinAddress(
  ...
  private TVMTestResult freezeForSelf(byte[] callerAddr,
    ...
    if (res == 0) {
      ...
      Assert.assertEquals(oldTotalNetWeight + frozenBalance / SYM_PRECISION,
      ...
      Assert.assertEquals(oldTotalEnergyWeight + frozenBalance / SYM_PRECISION,
  ...
  private TVMTestResult unfreezeForOther(byte[] callerAddr,
    ...
    // check total weight
    if (res == 0) {
      Assert.assertEquals(oldTotalNetWeight - delegatedFrozenBalance / SYM_PRECISION,
          dynamicStore.getTotalNetWeight());
      Assert.assertEquals(oldTotalEnergyWeight, dynamicStore.getTotalEnergyWeight());
    } else {
      Assert.assertEquals(oldTotalNetWeight, dynamicStore.getTotalNetWeight());
      Assert.assertEquals(oldTotalEnergyWeight - delegatedFrozenBalance / SYM_PRECISION,
          dynamicStore.getTotalEnergyWeight());
    }
  ...
  private TVMTestResult suicideToAccount(byte[] callerAddr,
    ...
    if (FastByteComparisons.isEqual(inheritorAddr,
      ...
      Assert.assertEquals(contract.getBalance() + contract.getKcoinPower(),
    ...
    Assert.assertEquals(contract.getFrozenBalance(),
        (oldTotalNetWeight - newTotalNetWeight) * SYM_PRECISION);
    Assert.assertEquals(contract.getEnergyFrozenBalance(),
        (oldTotalEnergyWeight - newTotalEnergyWeight) * SYM_PRECISION);
  ...
  private void checkReceipt(TVMTestResult result,
    ...
    long burnedSym = result.getReceipt().getEnergyFee();
