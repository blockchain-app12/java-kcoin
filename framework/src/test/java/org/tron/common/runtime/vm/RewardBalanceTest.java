  ...
  */
  ...
  public void testRewardBalance()
    ...
    Transaction sym = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] factoryAddress = WalletUtil.generateContractAddress(sym);
    ...
    sym = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    InternalTransaction rootInternalTransaction = new InternalTransaction(sym,
        InternalTransaction.SymType.SYM_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ProgramInvoke programInvoke = programInvokeFactory
        .createProgramInvoke(InternalTransaction.SymType.SYM_CONTRACT_CALL_TYPE,
            InternalTransaction.ExecutorType.ET_PRE_TYPE, sym,
    ...
    sym = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    rootInternalTransaction = new InternalTransaction(sym,
        InternalTransaction.SymType.SYM_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
