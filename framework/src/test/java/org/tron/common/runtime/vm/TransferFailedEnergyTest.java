...
import org.kcoin.common.runtime.TVMTestResult;
import org.kcoin.common.runtime.TvmTestUtils;
...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferSymInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendSymInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallSymInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateSymInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferSymNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallSymNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferSymSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendSymSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallSymInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferSymNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallSymNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowTvmConstantinop = {
      new TestCase("testTransferSymSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendSymSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferSymNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallSymNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowTvmConstantinop = {
      new TestCase("testTransferSymSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendSymSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferSymInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendSymInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateSymInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallSymInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  public void testTransferFailedAfterAllowTvmConstantinopl()
    ...
    // deploy contract
    Transaction sym = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, value, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(sym);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(sym, rootDeposit, null);
    ...
    sym = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(sym);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(sym, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
