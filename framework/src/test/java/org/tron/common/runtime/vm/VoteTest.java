...
import static org.kcoin.protos.Protocol.Transaction.Result.contractResult;
...
import org.kcoin.consensus.dpos.MaintenanceManager;
...
import stest.kcoin.wallet.common.client.utils.DataWord;
...
public class VoteTest {
  ...
  private static final long sym_precision = 1_000_000L;
  ...
  private byte[] deployContract(String contractName, String abi, String code) throws Exception {
    Protocol.Transaction sym = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null, 1_000_000L);
    sym = sym.toBuilder().setRawData(
        sym.getRawData().toBuilder().setTimestamp(System.currentTimeMillis()).build()).build();
  ...
  public void testRewardAlgorithmNo1() throws Exception {
    ...
    // cycle-1
    {
      // freeze balance to get kcoin power
  ...
  private void voteWitness(byte[] contract,
                           ...
                           List<Long> kcoinPowerList) throws Exception {
    triggerContract(contract, SUCCESS, getEqualConsumer(1),
        voteMethod, witnessList, kcoinPowerList);
  }
