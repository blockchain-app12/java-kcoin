...
public class TransactionCapsuleTest {
  ...
  public void addSign() {
    ...
    Transaction.Builder symBuilder = Transaction.newBuilder();
    ...
    symBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(symBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    symBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(symBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    symBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(symBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    symBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(symBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    symBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(symBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    symBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(symBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    symBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    symBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(symBuilder.build(), hash, accountStore);
    ...
    symBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(symBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    symBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(symBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    symBuilder = Transaction.newBuilder();
    ...
    symBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    symBuilder.clearSignature();
    symBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    symBuilder.clearSignature();
    symBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    symBuilder.clearSignature();
    symBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(symBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void symCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule symCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = symCap.getContractResult();
    symCap.resetResult();
    Assert.assertEquals(symCap.getInstance().getRetCount(), 0);
    symCap.setResultCode(contractResult);
    Assert.assertEquals(symCap.getInstance()
