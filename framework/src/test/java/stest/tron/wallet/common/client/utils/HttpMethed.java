...
public class HttpMethed {
  ...
  public static HttpResponse assetIssue(String httpNode, byte[] ownerAddress, String name,
      String abbr, Long totalSupply, Integer symNum, Integer num, Long startTime, Long endTime,
    ...
    try {
      ...
      userBaseObj2.addProperty("sym_num", symNum);
  ...
  public static HttpResponse freezeBalance(String httpNode, byte[] ownerAddress, Long frozenBalance,
    ...
    try {
      ...
      if (resourceCode == 2) {
        userBaseObj2.addProperty("resource", "KCOIN_POWER");
      }
   ...
   * constructor.
   */
  public static Long getBurnSym(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/wallet/getburnsym";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    responseContent = HttpMethed.parseResponseContent(response);
    return responseContent.getLong("burnSymAmount");
  }

  /**
   * constructor.
   */
  public static Long getBurnSymFromSolidity(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletsolidity/getburnsym";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
   ...
   * constructor.
   */
  public static Long getBurnSymFromPbft(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletpbft/getburnsym";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static HttpResponse sendShieldCoin(String httpNode, byte[] publicZenTokenOwnerAddress,
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        response = HttpMethed
            .getMerkleTreeVoucherInfo(httpNode, noteTx.getSymId(), noteTx.getIndex(), 1);
        responseContent = HttpMethed.parseResponseContent(response);
        ...
        List shieldedSpends = Lists.newArrayList(new JSONObjectWarp().put("note",
            ...
            .put("alpha", ByteArray.toHexString(org.kcoin.core.zen.note.Note.generateR()))
  ...
  public static Boolean getSpendResult(String httpNode, ShieldAddressInfo shieldAddressInfo,
    ...
    try {
      ...
      jsonObjectWarp.put("note", new JSONObjectWarp().put("value", noteTx.getValue())
          .put("payment_address", noteTx.getPaymentAddress())
          .put("rcm", ByteArray.toHexString(noteTx.getR()))
          .put("memo", ByteArray.toHexString(noteTx.getMemo()))).put("txid", noteTx.getSymId());
  ...
  public static List<ShieldNoteInfo> scanNoteByOvkFromPbft(String httpSolidityNode,
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        noteTx.setSymId(HttpMethed.parseStringContent(noteTxs).getString("txid"));
  ...
  public static HttpResponse sendShieldCoinWithoutAsk(String httpNode, String httpSolidityNode,
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        logger.info("noteTx.getSymId():" + noteTx.getSymId());
        HttpMethed.response = HttpMethed
            .getMerkleTreeVoucherInfoFromSolidity(httpSolidityNode, noteTx.getSymId(),
                noteTx.getIndex(), 1);
        ...
        HttpMethed.response = HttpMethed
            .getMerkleTreeVoucherInfoFromPbft(httpPbftNode, noteTx.getSymId(), noteTx.getIndex(),
                1);
        HttpMethed.responseContent = HttpMethed.parseResponseContent(HttpMethed.response);
        HttpMethed.printJsonContent(responseContent);
        ...
        alpha = org.kcoin.core.zen.note.Note.generateR();
