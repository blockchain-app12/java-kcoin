package stest.kcoin.wallet.dailybuild.http;
...
import stest.kcoin.wallet.common.client.utils.HttpMethed;
...
public class HttpTestGetAccountBalance001 {
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Get burn sym by http")
  public void test03GetBurnSym() {
    ...
    HttpMethed.waitToProduceOneBlock(httpnode);
    final Long beforeBurnSymAmount = HttpMethed.getBurnSym(httpnode);
    ECKey ecKey3 = new ECKey(Utils.getRandom());
    ...
    HttpMethed.waitToProduceOneBlockFromSolidity(httpnode, httpSolidityNode);
    Long afterBurnSymAmount = HttpMethed.getBurnSym(httpnode);
    logger.info(afterBurnSymAmount + "  :   " + beforeBurnSymAmount);
    Assert.assertTrue(afterBurnSymAmount - beforeBurnSymAmount == 100000L);

    Assert.assertEquals(afterBurnSymAmount, HttpMethed.getBurnSymFromSolidity(httpSolidityNode));
    Assert.assertEquals(afterBurnSymAmount, HttpMethed.getBurnSymFromPbft(httpPbftNode));
  }
