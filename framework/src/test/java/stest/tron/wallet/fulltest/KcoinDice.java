package stest.kcoin.wallet.fulltest;
...
public class KcoinDice {
  ...
  public void kcoinDice() {
    ECKey ecKey1 = new ECKey(Utils.getRandom());
    byte[] kcoinDiceAddress = ecKey1.getAddress();
    String kcoinDiceKey = ByteArray.toHexString(ecKey1.getPrivKeyBytes());
    PublicMethed
        .sendcoin(kcoinDiceAddress, 100000000000L, fromAddress, testKey002, blockingStubFull);
    String contractName = "KcoinDice";
    String code = Configuration.getByPath("testng.conf")
        .getString("code.code_KcoinDice_kcoinDice");
    String abi = Configuration.getByPath("testng.conf")
        .getString("abi.abi_KcoinDice_kcoinDice");
    byte[] contractAddress = PublicMethed.deployContract(contractName, abi, code, "",
        maxFeeLimit, 1000000000L, 100, null, kcoinDiceKey, kcoinDiceAddress, blockingStubFull);
    ...
    for (Integer i = 0; i < 100; i++) {
      ...
      txid = PublicMethed.triggerContract(contractAddress,
          ...
          1000000, maxFeeLimit, kcoinDiceAddress, kcoinDiceKey, blockingStubFull);
      logger.info(txid);
      txidList.add(txid);
