...
contract KittyBase is KittyAccessControl {
    ...
    event Birth(address owner, uint256 kittyId, uint256 makcoinId, uint256 sireId, uint256 genes);
    ...
    struct Kitty {
        ...
        // timer (for makcoins) as well as the siring cooldown.
        ...
        // Set to the ID of the sire cat for makcoins that are pregnant,
        ...
        // of whether this cat is acting as makcoin or sire.
        ...
        // (i.e. max(makcoin.generation, sire.generation) + 1)
        uint16 generation;
    }
    ...
    ///  breeding action, called "pregnancy time" for makcoins and "siring cooldown"
    ...
    ///  creature that is both makcoin and sire... to itself! Has an invalid genetic code.
    ...
    /// @param _makcoinId The kitty ID of the makcoin of this cat (zero for gen0)
    ...
    returns (uint)
    {
        ...
        require(_makcoinId == uint256(uint32(_makcoinId)));
...
abstract contract ERC721 {
    // Required methods
    function totalSupply() public virtual view returns (uint256 total);
...
contract KittyBreeding is KittyOwnership {
    ...
    ///  timer begins for the makcoin.
    event Pregnant(address owner, uint256 makcoinId, uint256 sireId, uint256 cooldownEndBlock);
    ...
    /// @dev Check if a sire has authorized breeding with this makcoin. True if both sire
    ///  and makcoin have the same owner, or if the sire has given siring permission to
    ///  the makcoin's owner (via approveSiring()).
    function _isSiringPermitted(uint256 _sireId, uint256 _makcoinId) internal view returns (bool) {
        address makcoinOwner = kittyIndexToOwner[_makcoinId];
        address sireOwner = kittyIndexToOwner[_sireId];

        // Siring is okay if they have same owner, or if the makcoin's owner was given
        // permission to breed with this sire.
        return (makcoinOwner == sireOwner || sireAllowedToAddress[_sireId] == makcoinOwner);
    }
    ...
    ///  period has passed.
    function _isReadyToGiveBirth(Kitty memory _makcoin) private view returns (bool) {
        return (_makcoin.siringWithId != 0) && (_makcoin.cooldownEndBlock <= uint64(block.number));
    }
    ...
    /// @dev Internal check to see if a given sire and makcoin are a valid mating pair. DOES NOT
    ///  check ownership permissions (that is up to the caller).
    /// @param _makcoin A reference to the Kitty struct of the potential makcoin.
    /// @param _makcoinId The makcoin's ID.
    /// @param _sire A reference to the Kitty struct of the potential sire.
    /// @param _sireId The sire's ID
    function _isValidMatingPair(
        Kitty storage _makcoin,
        uint256 _makcoinId,
        Kitty storage _sire,
        uint256 _sireId
    )
    ...
    returns(bool)
    {
        ...
        if (_makcoinId == _sireId) {
            return false;
        }

        // Kitties can't breed with their parents.
        if (_makcoin.makcoinId == _sireId || _makcoin.sireId == _sireId) {
            return false;
        }
        if (_sire.makcoinId == _makcoinId || _sire.sireId == _makcoinId) {
            return false;
        }

        // We can short circuit the sibling check (below) if either cat is
        // gen zero (has a makcoin ID of zero).
        if (_sire.makcoinId == 0 || _makcoin.makcoinId == 0) {
            return true;
        }

        // Kitties can't breed with full or half siblings.
        if (_sire.makcoinId == _makcoin.makcoinId || _sire.makcoinId == _makcoin.sireId) {
            return false;
        }
        if (_sire.sireId == _makcoin.makcoinId || _sire.sireId == _makcoin.sireId) {
            return false;
        }
    ...
    /// @dev Internal check to see if a given sire and makcoin are a valid mating pair for
    ///  breeding via auction (i.e. skips ownership and siring approval checks).
    function _canBreedWithViaAuction(uint256 _makcoinId, uint256 _sireId)
    internal
    view
    returns (bool)
    {
        Kitty storage makcoin = kitties[_makcoinId];
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(makcoin, _makcoinId, sire, _sireId);
    }
    ...
    /// @param _makcoinId The ID of the proposed makcoin.
    /// @param _sireId The ID of the proposed sire.
    function canBreedWith(uint256 _makcoinId, uint256 _sireId)
    external
    view
    returns(bool)
    {
        require(_makcoinId > 0);
        require(_sireId > 0);
        Kitty storage makcoin = kitties[_makcoinId];
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(makcoin, _makcoinId, sire, _sireId) &&
        _isSiringPermitted(_sireId, _makcoinId);
    }

    /// @dev Internal utility function to initiate breeding, assumes that all breeding
    ///  requirements have been checked.
    function _breedWith(uint256 _makcoinId, uint256 _sireId) internal {
        // Grab a reference to the Kitties from storage.
        Kitty storage sire = kitties[_sireId];
        Kitty storage makcoin = kitties[_makcoinId];

        // Mark the makcoin as pregnant, keeping track of who the sire is.
        makcoin.siringWithId = uint32(_sireId);

        // Trigger the cooldown for both parents.
        _triggerCooldown(sire);
        _triggerCooldown(makcoin);

        // Clear siring permission for both parents. This may not be strictly necessary
        // but it's likely to avoid confusion!
        delete sireAllowedToAddress[_makcoinId];
        ...
        emit Pregnant(kittyIndexToOwner[_makcoinId], _makcoinId, _sireId, makcoin.cooldownEndBlock);
    }

    /// @notice Breed a Kitty you own (as makcoin) with a sire that you own, or for which you
    ///  have previously been given Siring approval. Will either make your cat pregnant, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBirth()
    /// @param _makcoinId The ID of the Kitty acting as makcoin (will end up pregnant if successful)
    /// @param _sireId The ID of the Kitty acting as sire (will begin its siring cooldown if successful)
    function breedWithAuto(uint256 _makcoinId, uint256 _sireId)
    external
    payable
    whenNotPaused
    {
        ...
        // Caller must own the makcoin.
        require(_owns(msg.sender, _makcoinId));

        // Neither sire nor makcoin are allowed to be on auction during a normal
        // breeding operation, but we don't need to check that explicitly.
        // For makcoin: The caller of this function can't be the owner of the makcoin
        ...
        // Check that makcoin and sire are both owned by caller, or that the sire
        // has given siring permission to caller (i.e. makcoin's owner).
        // Will fail for _sireId = 0
        require(_isSiringPermitted(_sireId, _makcoinId));

        // Grab a reference to the potential makcoin
        Kitty storage makcoin = kitties[_makcoinId];

        // Make sure makcoin isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(makcoin));
        ...
        _breedWith(_makcoinId, _sireId);
    }

    /// @notice Have a pregnant Kitty give birth!
    /// @param _makcoinId A Kitty ready to give birth.
    ...
    ///  to the current owner of the makcoin. Upon successful completion, both the makcoin and the
    ///  new kitten will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new kitten always goes to the mother's owner.
    function giveBirth(uint256 _makcoinId)
    external
    whenNotPaused
    returns(uint256)
    {
        // Grab a reference to the makcoin in storage.
        Kitty storage makcoin = kitties[_makcoinId];

        // Check that the makcoin is a valid cat.
        require(makcoin.birthTime != 0);

        // Check that the makcoin is pregnant, and that its time has come!
        require(_isReadyToGiveBirth(makcoin));

        // Grab a reference to the sire in storage.
        uint256 sireId = makcoin.siringWithId;
        Kitty storage sire = kitties[sireId];

        // Determine the higher generation number of the two parents
        uint16 parentGen = makcoin.generation;
        if (sire.generation > makcoin.generation) {
            parentGen = sire.generation;
        }

        // Call the sooper-sekret gene mixing operation.
        uint256 childGenes = geneScience.mixGenes(makcoin.genes, sire.genes, makcoin.cooldownEndBlock - 1);

        // Make the new kitten!
        address owner = kittyIndexToOwner[_makcoinId];
        uint256 kittenId = _createKitty(_makcoinId, makcoin.siringWithId, parentGen + 1, childGenes, owner);

        // Clear the reference to sire from the makcoin (REQUIRED! Having siringWithId
        // set is what marks a makcoin as being pregnant.)
        delete makcoin.siringWithId;
...
contract KittyAuction is KittyBreeding {
    ...
    ///  Immediately breeds the winning makcoin with the sire on auction.
    /// @param _sireId - ID of the sire on auction.
    /// @param _makcoinId - ID of the makcoin owned by the bidder.
    ...
    whenNotPaused
    {
        // Auction contract checks input sizes
        require(_owns(msg.sender, _makcoinId));
        require(isReadyToBreed(_makcoinId));
        require(_canBreedWithViaAuction(_makcoinId, _sireId));
        ...
        _breedWith(uint32(_makcoinId), uint32(_sireId));
    }
...
contract KittyCore is KittyMinting {
    ...
    returns (
        ...
        makcoinId = uint256(kit.makcoinId);
