...
# Function: to start, stop and restart java-kcoin.
...
JAR_FILE_PATH="./build/libs/java-kcoin.jar"
PID_FILE_PATH="java-kcoin.pid"
LOG_FILE_PATH="java-kcoin.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-kcoin."
        else
            echo "Failed to start java-kcoin."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-kcoin."
        else
            echo "Failed to stop java-kcoin."
        fi
    ;;
    restart)
